...
from . import write_specs
from .base import Context, ToolSpec
from .write_specs import DRAFT_MODES as _DRAFT_MODES
from .write_specs import MAX_BULK_IDS
from .write_specs import RESPONSE_METHOD as _RESPONSE_METHOD
...
# --- the pack: metadata from write_specs, behaviour bound here ----------------
#
# The specs live in write_specs.py (exchangelib-free, so the thin MCP can
# import them); this is where each one gets its real handler, and where
# send_draft gets back the confirm predicate that skips the gate on an
# idempotent replay. A missing entry raises at import time, so a new tool
# can never ship with the placeholder handler still attached.

_HANDLERS: Dict[str, Any] = {
    "create_draft": _create_draft,
    "update_draft": _update_draft,
    "create_upload_link": _create_upload_link,
...
_PREVIEWS: Dict[str, Any] = {"send_draft": _send_draft_preview}
_CONFIRMS: Dict[str, Any] = {"send_draft": _send_confirm_needed}

assert set(_OOF_STATE) == set(write_specs.OOF_STATES), "OOF states drifted"

TOOLS: List[ToolSpec] = [
    replace(spec,
            handler=_HANDLERS[spec.name],
            preview=_PREVIEWS.get(spec.name, spec.preview),
            confirm=_CONFIRMS.get(spec.name, spec.confirm))
    for spec in write_specs.SPECS
]
assert len(TOOLS) == len(_HANDLERS), "a write spec has no handler bound"
