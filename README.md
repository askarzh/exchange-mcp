...
| `GEMINI_CLEAN_MODEL` | `gemini-3.5-flash-lite` | Model the boundary detector asks for the paragraph index where boilerplate begins (temperature 0, JSON `responseSchema`). Any answer that is not a validated in-range index is logged as an error and can never cut a paragraph |
